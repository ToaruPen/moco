...
ローカル音声エージェントです。設定したトークキーを押している間だけマイクを有効にし、
離すと発話を確定します。応答や読み上げは設定したキャンセルキーで停止できます。
...
ページで「音声卓を有効にする」を押し、Chrome にマイクを許可してください。初期設定の
...
設定したトークキーを再び押すと、再生中の古い音声を止めてから次の発話を受け付けます。
設定したアイドル時間を過ぎると会話だけが閉じられ、デーモンと操作ページは残ります。
次の push-to-talk で新しい会話が作られ、以前の文字起こしは自動投入されません。
